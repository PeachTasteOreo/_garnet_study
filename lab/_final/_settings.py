...
def mode(portHandler, packetHandler, id, mode_number):
    ...
    # Set Operating Mode
    result, error = packetHandler.write1ByteTxRx(portHandler, id , ADDR_OPERATE_MODE, mode_number) #2Bybte?
    if result != COMM_SUCCESS:
        ...
        result, error = packetHandler.write1ByteTxRx(portHandler, id, ADDR_TORQUE_ENABLE, TORQUE_ENABLE)

        print("from settigns : \"result\"  : %s" % packetHandler.getTxRxResult(result))
        print("from settings : \"error\" : %s" % packetHandler.getRxPacketError(error))
